    ...
    <div className="w-full overflow-hidden relative" dir="ltr" style={{ maskImage: "linear-gradient(to right, transparent 0%, black 8%, black 92%, transparent 100%)", WebkitMaskImage: "linear-gradient(to right, transparent 0%, black 8%, black 92%, transparent 100%)" }}>
      ...
      <div className="marquee-track">
        {items.map((cat, i) => (
          ...
          >
            <div className="w-20 h-20 sm:w-28 sm:h-28 rounded-full bg-transparent border-2 border-gray-200 group-hover:border-[#A842E4] overflow-hidden relative transition-all duration-200 shadow-sm group-hover:shadow-md">
              {cat.image ? (
                <Image
                  src={cat.image}
                  alt={cat.name}
            ...
            <p className="text-xs sm:text-sm font-medium text-gray-700 text-center leading-tight line-clamp-2 group-hover:text-[#8543C0] transition-colors w-full" dir="rtl">
              {cat.name}
            </p>
          </Link>
        ))}
