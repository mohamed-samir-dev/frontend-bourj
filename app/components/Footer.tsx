...
export default async function Footer() {
  ...
  return (
    <footer className="text-gray-100 mt-16" style={{ background: 'linear-gradient(to bottom, #7A2FCC, #611FA0)' }} dir="rtl">
      <div className="max-w-6xl mx-auto px-4 py-10 grid grid-cols-1 sm:grid-cols-3 gap-8">

        {/* من نحن */}
        <div>
          <h3 className="text-[#A842E4] font-bold text-xl mb-3">من نحن</h3>
          <p className="text-sm leading-relaxed text-[#eddff9]">
        ...
        {/* روابط مهمة */}
        <div>
          <h3 className="text-[#A842E4] font-bold text-xl mb-4">روابط مهمة</h3>
          <ul className="space-y-2.5 text-sm">
            {[
              ...
              <li key={href}>
                <Link href={href} className="hover:text-[#A842E4] transition-colors">
                  {label}
                </Link>
              </li>
            ))}
        ...
        {/* تواصل معنا */}
        <div>
          <h3 className="text-[#A842E4] font-bold text-xl mb-4">تواصل معنا</h3>
          <ul className="space-y-2.5 text-sm mb-5">
            {c.whatsapp && (
              <li className="flex items-center gap-2">
                <FaWhatsapp className="text-emerald-500 shrink-0" size={16} />
                <a href={`https://wa.me/${c.whatsapp.replace(/\D/g, "")}`} target="_blank" rel="noreferrer" className="hover:text-[#A842E4] transition-colors" dir="ltr">{c.whatsapp}</a>
              </li>
            )}
            {c.phone && (
              <li className="flex items-center gap-2">
                <FaMobileAlt className="text-emerald-500 shrink-0" size={16} />
                <a href={`tel:${c.phone}`} className="hover:text-[#A842E4] transition-colors" dir="ltr">{c.phone}</a>
              </li>
            )}
            {c.email && (
              <li className="flex items-center gap-2">
                <FaEnvelope className="text-emerald-500 shrink-0" size={16} />
                <a href={`mailto:${c.email}`} className="hover:text-[#A842E4] transition-colors" dir="ltr">{c.email}</a>
              </li>
            )}
          </ul>
      ...
      <div className="border-t border-[#8543C0] flex flex-col sm:flex-row items-center justify-between max-w-6xl mx-auto px-4 py-4 text-xs text-[#eddff9] gap-3">
